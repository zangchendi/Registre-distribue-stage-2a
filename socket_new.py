...
class Peer:
	...
	def receive_bid(self,data):
		content=data.split()
		m=Message(content[3],content[-1],content[-3],True, False)
		if m.current_round!=self.get_current_round:
			return
		for bid in self.bids:
			if bid.username==m.username:
				return
...
if __name__ == '__main__':
	thread=[]
	t=Thread(target=alice.receive)
	thread.append(t)
	bid=alice.input_bid(bids, Peer_list, alice.get_current_round())
	t=Thread(target=alice.send_all, args=(bid, Peer_list))
	thread.append(t)
	for t in thread:
		t.start()	
		time.sleep(5)				
